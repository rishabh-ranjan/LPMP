...
namespace LPMP {
    ...
    asymmetric_multiway_cut_labeling asymmetric_multiway_cut_gaec(const asymmetric_multiway_cut_instance& instance)
    {
        ...
        for(std::size_t e=0; e<nr_edges; ++e)
        {
            const std::size_t i = instance.edge_costs.edges()[e][0];
            const std::size_t j = instance.edge_costs.edges()[e][1];
            const std::size_t c_i = partition.find(i);
            const std::size_t c_j = partition.find(j);
            const std::size_t l_i = node_labels[c_i];
            const std::size_t l_j = node_labels[c_j];
