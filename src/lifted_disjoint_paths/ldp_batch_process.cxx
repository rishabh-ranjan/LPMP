...
LdpBatchProcess::LdpBatchProcess(VertexGroups<>& shiftedGroups, std::vector<std::array<std::size_t,2>>& shiftedVertexLabels_, std::size_t maxLabelSoFar_, std::size_t maxTimeForLabeled_, std::size_t minTimeToUse, std::size_t maxTimeToUse){ //shifted vertex labels assume that vertex with zero index is minVertex in VG
  constructorBegin=std::chrono::steady_clock::now();
    std::size_t unset=std::numeric_limits<std::size_t>::max();
    pvg=&shiftedGroups;
   ...
   // origVerticesScore=std::vector<double>(numberOfVerticesInBatch);
    ...
    if(minValidVertex>minVertex){
        ...
        shiftedLabels=std::vector<std::size_t>(minValidVertex-minVertex);
        for (std::size_t i = 0; i < shiftedVertexLabels_.size(); ++i) {
            std::size_t vertex=shiftedVertexLabels_[i][0];
...
void LdpBatchProcess::createLocalVG(LPMP::VertexGroups<>& localVG){
    {
            ...
            std::size_t layerCounter=1;
             std::size_t lastVertex=0;
            if(numberOfUsedLabels>0){
                groups[1]=std::vector<std::size_t>(numberOfUsedLabels);
                for (std::size_t i = 0; i < numberOfUsedLabels; ++i) {
                    groups[1][i]=i;
            ...
            std::vector<std::size_t> currenGroup;
            for (std::size_t i = maxTimeForLabeled+1; i <= maxTime; ++i) {
                const std::vector<std::size_t>& verticesGlobal= pvg->getGroupVertices(i);
                for (std::size_t j = 0; j < verticesGlobal.size(); ++j) {
                    std::size_t globalID=verticesGlobal.at(j);
                    std::size_t localID=globalIndexToLocalIndex(globalID);
                ...
                currenGroup=std::vector<std::size_t>();
                layerCounter++;
            }
            std::size_t s=lastVertex+1;
            std::size_t t=lastVertex+2;
            std::vector<std::size_t> sGroup={s};
            std::vector<std::size_t> tGroup={t};
...
LdpBatchProcess::LdpBatchProcess(VertexGroups<>& shiftedGroups, std::vector<std::array<std::size_t,2>>& shiftedVertexLabels_, std::size_t maxLabelSoFar_, std::size_t maxTimeForLabeled_){ //shifted vertex labels assume that vertex with zero index is minVertex in VG
    std::size_t unset=std::numeric_limits<std::size_t>::max();
    LdpBatchProcess(shiftedGroups,shiftedVertexLabels_,maxTimeForLabeled_,maxLabelSoFar_,unset,unset);
}


void LdpBatchProcess::decode(const std::vector<std::vector<std::size_t>>& paths){
    std::size_t globalLabelCounter=maxLabelSoFar+1;
    decodedLabels=std::vector<std::array<std::size_t,2>>();
    std::vector<std::size_t> localLabels(numberOfOutputVertices);
    for (std::size_t i = 0; i < paths.size(); ++i) {
        std::size_t firstVertex=paths.at(i).at(0);
        std::size_t labelToUse=0;
    ...
    for (int i = numberOfUsedLabels; i < numberOfOutputVertices; ++i) {
        std::size_t globalID=localIndexToGlobalIndex(i);
        std::size_t label=localLabels[i];
        if(label>0){
...
void LdpBatchProcess::initVertexScoreFromVector(const std::vector<std::size_t>& vertexList,const std::vector<double>& costs){
    ...
    while(i<vertexList.size()){
       // std::cout<<"processing vertex "<<i<<std::endl;
        std::size_t vertex=vertexList[i];
        if(vertex<minValidVertex){
            i++;
            continue;
        }
        if(vertex>maxVertex) break;
        std::size_t localIndex=globalIndexToLocalIndex(vertex);
...
std::size_t LdpBatchProcess::globalIndexToLocalIndex(const std::size_t& globalIndex){  //only for vertices from valid layers
    assert(globalIndex>=minValidVertex&&globalIndex<=maxVertex);
    //assert(numberOfUsedLabels<std::numeric_limits<std::size_t>::max());
    std::size_t toReturn=globalIndex+numberOfUsedLabels-minValidVertex;
    //std::cout<<"to return "<<toReturn<<std::endl;
   // assert(localIndexToGlobalIndex(toReturn)==globalIndex);
    return toReturn;
}

std::size_t LdpBatchProcess::localIndexToGlobalIndex(const std::size_t &localIndex){
    assert(localIndex>=numberOfUsedLabels&&localIndex<numberOfOutputVertices);
    std::size_t toReturn=localIndex+minValidVertex-numberOfUsedLabels;
   // assert(globalIndexToLocalIndex(toReturn)==localIndex);
    return toReturn;
}

void LdpBatchProcess::initEdgesFromVector(const std::vector<std::array<std::size_t,2>>& edges,const std::vector<double> &costs){

    std::size_t i = 0;
    std::vector<std::array<std::size_t,2>> outputEdges;
    ...
    while (i < edges.size()) {
        ...
        std::size_t time0=pvg->getGroupIndex(v0);
        std::size_t time1=pvg->getGroupIndex(v1);
        assert(time0<time1);
        ...
        std::size_t vertexIndexInLabels=v0-minVertex;
        assert(vertexIndexInLabels<shiftedLabels.size());
        std::size_t label=shiftedLabels[vertexIndexInLabels];
        if(label>0){
   ...
   // std::cout<<"map finished, used labels "<<numberOfUsedLabels<<std::endl;
    localIndexToGlobalLabel=std::vector<std::size_t>(numberOfUsedLabels);
   ...
   // std::cout<<"number of output vertices "<<numberOfOutputVertices<<std::endl;
    ...
    for (auto iter=edgesFromLabeled.begin();iter!=edgesFromLabeled.end();iter++) {
       // std::cout<<"in for"<<std::endl;
        ...
        for(auto iter2=neighbors.begin();iter2!=neighbors.end();iter2++){
            std::size_t vertexGlobID=iter2->first;
            double cost=iter2->second;
            std::size_t vertexLocalID=globalIndexToLocalIndex(vertexGlobID);
    ...
    //    std::cout<<v0<<","<<v1<<std::endl;
     ...
     //   std::cout<<"after assert"<<std::endl;
        ...
        std::size_t transformedV0=globalIndexToLocalIndex(v0);
        std::size_t transformedV1=globalIndexToLocalIndex(v1);
