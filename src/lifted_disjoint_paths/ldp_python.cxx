...
PYBIND11_MODULE(ldpMessagePassingPy, m) {
     ...
     py::class_<LPMP::lifted_disjoint_paths::LdpParameters<std::size_t>>(m, "LdpParams")
     ...
     py::class_<LPMP::VertexGroups<>>(m, "TimeFramesToVertices")
        ...
        .def("init_from_vector", py::overload_cast<const std::vector<std::size_t>&>(&LPMP::VertexGroups<>::initFromVector), "Initializes vertices in time frames from a vector of std::size_t")
        .def("init_from_vector_shift", py::overload_cast<const std::vector<std::size_t>&,std::size_t ,std::size_t>(&LPMP::VertexGroups<>::initFromVector), "Initializes vertices in time frames from a vector of std::size_t,time shift and vertex shift")
     ...
     py::class_<LPMP::CompleteStructure<>>(m, "GraphStructure")
            ...
            // .def("add_edges_from_array", &LPMP::CompleteStructure<>::addEdgesFromMatrix, "Initializes edges of the graph between two time frames from a matrix.")
             ...
             .def("add_edges_from_vectors", &LPMP::CompleteStructure<>::addEdgesFromVectors<LPMP::lifted_disjoint_paths::LdpParameters<>>, "Initializes edges of the graph from an Nx2 array of std::size_t with edge vertices and an Nx1 array of doubles with costs. Restrictions on maximal vertex and maximal time gap from parameters apply.")
             .def("add_edges_from_vectors_all", &LPMP::CompleteStructure<>::addEdgesFromVectorsAll, "Initializes edges of the graph from an Nx2 array of std::size_t with edge vertices and an Nx1 array of doubles with costs. All edges added. No restriction on maximal timegap. ")
     ...
     py::class_<LPMP::lifted_disjoint_paths::LdpInstance>(m, "LdpInstance")
             .def(py::init<LPMP::lifted_disjoint_paths::LdpParameters<std::size_t> &,LPMP::CompleteStructure<>&>())
             .def(py::init<LPMP::lifted_disjoint_paths::LdpParameters<std::size_t>&,LPMP::LdpBatchProcess&>())
             .def(py::init<LPMP::lifted_disjoint_paths::LdpParameters<std::size_t>&,LPMP::LdpIntervalConnection&>())
             .def(py::init<LPMP::lifted_disjoint_paths::LdpParameters<>&,const py::array_t<std::size_t>&,const py::array_t<std::size_t>&,const  py::array_t<double>& ,const  py::array_t<double>&,const  py::array_t<double>&,LPMP::VertexGroups<>&>()) ;
     ...
     m.def("get_base_edge_labels",&LPMP::getBaseEdgeLabels<std::vector<std::array<std::size_t,2>>>,"Given a vector of base edge vertices, vector of solution paths and the number of graph vertices, it returns labels to base edges.");

     m.def("get_lifted_edge_labels",&LPMP::getLiftedEdgeLabels<std::vector<std::array<std::size_t,2>>>,"Given a vector of lifted edge vertices, vector of solution paths and the number of graph vertices, it returns labels to base edges.");


     py::class_<LPMP::LdpBatchProcess>(m,"BatchProcess")
             .def(py::init<LPMP::VertexGroups<>&, std::vector<std::array<std::size_t,2>>&, std::size_t, std::size_t, std::size_t, std::size_t>(),"TimeFramesToVertices,n x 2 vector of vertex ID->label, max used label, max time for using labels,min time of batch, max time of batch")
     ...
     py::class_<LPMP::LdpPathsExtractor>(m,"PathsExtractor")
             .def(py::init<LPMP::VertexGroups<>&,const std::vector<std::vector<std::size_t>>&, std::size_t,bool,bool,std::size_t>(),"Requires: TimeFramesToVertices of the respective interval, solution paths,length to cut off, bool is this first interval,bool is this last interval,vertex shift (global id of the first valid vertex)")
