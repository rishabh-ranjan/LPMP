...
namespace LPMP {
    ...
    {
        const std::size_t nr_nodes = instance.nr_nodes();
        const std::size_t nr_edges = instance.nr_edges();
        const std::size_t nr_labels = instance.nr_labels();
        constexpr static std::size_t no_label = std::numeric_limits<std::size_t>::max();
        ...
        auto label_cost = [&](const std::size_t i, const std::size_t l) -> double& {
            assert(i < nr_nodes && l < nr_labels);
            const std::size_t i_c = partition.find(i);
            return label_costs[i_c*nr_labels + l]; 
        };

        // first, label each node with its most probable class.
        std::vector<std::size_t> node_labels(nr_nodes, no_label);
        std::vector<std::size_t> nr_clusters_per_labels(nr_labels, 0); // nr clusters with specific label
        for(std::size_t i=0; i<nr_nodes; ++i)
        {
            double min_label_cost = std::numeric_limits<double>::infinity();
            std::size_t min_label = 0;
            for(std::size_t l=0; l<nr_labels; ++l)
            {
        ...
        auto compute_edge_cost = [&](const double mc_edge_cost, const std::size_t i, const std::size_t j) -> std::tuple<double,std::size_t> {
            // cost of join nodes, assign partition of joint minimum cost
            const std::size_t i_c = partition.find(i);
            const std::size_t j_c = partition.find(j);
            double min_label_cost = std::numeric_limits<double>::infinity();
            std::size_t min_label = 0;
            for(std::size_t l=0; l<nr_labels; ++l)
            {
        ...
        while(!Q.empty()) {
            ...
            const std::size_t c_i = partition.find(i);
            const std::size_t i_label = node_labels[c_i];
            const std::size_t c_j = partition.find(j);
            const std::size_t j_label = node_labels[c_j];
            ...
            partition.merge(i,j);
            const std::size_t c_ij = partition.find(i);
            for(std::size_t l=0; l<nr_labels; ++l)
            ...
            const std::size_t stable_label = node_labels[partition.find(stable_node)];
            ...
            for(std::size_t edge_index=g.first_outgoing_edge_index(merge_node); edge_index!=decltype(g)::no_next_edge; edge_index=g.next_outgoing_edge_index(edge_index)) {
                ...
                if(head == stable_node)
                    continue;
                const std::size_t head_label = node_labels[partition.find(head)];
