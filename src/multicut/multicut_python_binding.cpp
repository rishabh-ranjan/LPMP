...
void construct_instance(LPMP::multicut_instance& instance, const Eigen::Array<std::size_t, Eigen::Dynamic, 2>& edge_indices, const Eigen::Array<double, Eigen::Dynamic, 1>& edge_costs)
{
...
py::array_t<char> get_edge_mask(const LPMP::multicut_instance& instance, const LPMP::multicut_edge_labeling& edge_labels)
{
    ...
    for(std::size_t e=0; e<instance.no_edges(); ++e)
        edge_mask[e] = edge_labels[e];

    return py::array({pybind11::sstd::size_t(instance.no_edges())}, edge_mask); 
}
...
py::array_t<int> get_cc_ids_mask(const LPMP::multicut_instance& instance, const std::vector<int>& node_labeling)
{
    ...
    return py::array({pybind11::sstd::size_t(instance.no_nodes())}, cc_mask); 
} 
...
PYBIND11_MODULE(multicut_py, m) {
    ...
    py::class_<LPMP::multicut_instance>(m, "multicut_instance")
        .def(py::init<>())
        .def(py::init([](const Eigen::Array<std::size_t, Eigen::Dynamic, 2>& edge_indices, const Eigen::Array<double, Eigen::Dynamic, 1>& edge_costs) {
